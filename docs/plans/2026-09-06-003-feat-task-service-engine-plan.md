...
Inspected 2026-09-09: clean `main` at `679f5cefd6e626a3c67c1e08a433ab786c944883`. `go.mod` selects Go 1.25.0, modernc SQLite v1.58.0 and libc v1.75.6. `internal/core`, `internal/ports` and `internal/storage` exist; `internal/service` and the inbound port do not. No new dependency is needed for this feature.
...
- **Verification:** `make test-unit` contains focused assertions; `make test`, `make race`, `make validate` prove the boundary including storage faults. 003-V01–003-V12 and 003-V88. No future Make filtering variables are assumed.
