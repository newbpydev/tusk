...
| **CORE-ISS-007** | Doc Review | Error Taxonomy / Validation | P2 | Fixed in Plan | Split progress setters lacked typed sentinel error and contract for manual vs rollup progress. | Added `ErrInvalidProgress` sentinel, `SetProgress` (0–99 on non-done, 100 on done), and `SetRollupProgress` (0–100 for parent rollup). | Unit 001-3 tests (`TestTask_SetProgress_Validation`, `TestTask_SetRollupProgress`, `CORE-ERR-N1`). |
...
- **Decision & Fix**: Added immutable `ErrInvalidProgress = Error("task progress must be an integer between 0 and 100")`. Specified `SetProgress` for manual leaf updates ($0 \le \text{progress} \le 99$ on non-done tasks, $100$ strictly on done tasks) and `SetRollupProgress` for service-layer rollup calculations ($0 \le \text{progress} \le 100$ on parents whose subtasks are done).
