...
- [x] **CORE-TSK-R1 Recovery / Reopen**: Moving status from `StatusDone` to `StatusInProgress` sets `CompletedAt = nil`, `IsDone() == false`, and updates `UpdatedAt`. Transition to `StatusDone` sets `IsDone() == true`. Idempotent same-status calls repair stale lifecycle fields (restoring `CompletedAt`, preserving valid 0–99 progress) while leaving consistent tasks untouched. `SetParent` updates `ParentID`, `IsRoot() == (ParentID == nil)`, and `UpdatedAt`, rejecting self-parenting with `ErrSelfParenting`.
